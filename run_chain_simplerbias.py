...
from like_wx_simpler_bias import *
from deconv import *
from like_tf import *
...
def run_chain(fname_deconv):
    ...
    with open(fname_deconv, 'r') as infile:
        deconvolution_res = json.load(infile)
    ...
    def propose_nz(idx, vec_s):
        list_step_nz = 8.*np.array([0.04302523, 0.01372454, 0.00620927,
                                 0.001084058, 0.001117412, 0.001068708,
                                 0.00107255,  0.001219014, 0.005442341,
                                 0.02102579, 0.01899059, 0.02605574,
                                 0.05406305, 0.05852213, 0.05649834,
                                 0.07510255, 0.08012916, 0.09370887,
                                 0.022215714])
    ...
    def propose_b(idx, vec_b):
        list_step_bias = np.array([0.01, 0.01, 0.02])   #, 0.01, 0.01, 0.02, 0.01, 0.01, 0.02])
        bias_proposed = np.random.normal(vec_b[idx], list_step_bias[idx])
        res_vec_b = np.copy(vec_b)
        res_vec_b[idx] = bias_proposed
        return res_vec_b

    def loglike(nz_s, vec_b, idx_model):
        pi_curr = backtransform_logit(nz_s)
        return model_tf.loglike(nz_s, idx_model) + model_loglike_wx.log(pi_curr, vec_b)
    ...
    for idx_samp in range(n_samp):
        if idx_samp%100 == 0:
            ...
            np.savetxt(X=trace_vec_s_out, fname=fname_deconv+str(trace_idx_model[-1]) + str(wxcov_mod)+'simpler_bias_mod_chain1_trace_vec_s_test_large_std_bratio_second_DESI.dat')
            np.savetxt(X=trace_bias_out, fname=fname_deconv+str(trace_idx_model[-1]) +str(wxcov_mod)+'simpler_bias_mod_chain1_trace_bias_test_large_std_bratio_second_DESI.dat')
            np.savetxt(X=trace_loglike_out, fname=fname_deconv+str(trace_idx_model[-1]) +str(wxcov_mod)+'simpler_bias_mod_chain1_trace_loglike_bratio_second_DESI.dat')

        vec_s = np.copy(trace_vec_s[-1])
        for nz_idx in range(len_vec_s):
            proposed_nz = propose_nz(nz_idx, vec_s)

            log_new = loglike(proposed_nz, trace_bias[-1], trace_idx_model[-1])
            log_old = loglike(vec_s, trace_bias[-1], trace_idx_model[-1])
        ...
        for b_idx in range(len_vec_b):
            ...
            log_new = loglike(trace_vec_s[-1], proposed_b, trace_idx_model[-1])
            log_old = loglike(trace_vec_s[-1], vec_b, trace_idx_model[-1])
            log_accept_ratio = log_new - log_old
            rnd_curr = np.log(np.random.uniform(low=0.0, high=1.0))
            if log_accept_ratio > rnd_curr:
                vec_b = proposed_b
...
list_dec = ['output_regularization_models.json']
